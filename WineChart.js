...
wineChart = {
    ...
    const t = group.transition().duration(750); // define the main svg group transition by using the transition() method specifying the duration with duration() method
  
    // transition all arcs and labels (including hidden arcs & labels)
  
    path
      .transition(t) // initiate transition of path group
      ...
      .attr(
        "fill-opacity",
        (
          d // apply opacity to filtered elements
        ) => (arcVisible(d.target) ? (d.children ? 0.7 : 0.5) : 0)
      )
      .attr("pointer-events", (d) => (arcVisible(d.target) ? "auto" : "none")) // set pointer events attribute to filtered elements
    ...
    label
      ...
      .transition(t) // initiate transition of label group
    ...
    function labelTransform(d) {
      // calclate the transformation needed to position the label correctly on the arc
